...
namespace {
    ...
    private:
        ...
        void load_gde2_version()
        {
            xmlDocPtr document;
            xmlXPathContextPtr context;
            const string nodes[3] = { "string(/gde2-version/platform)",
                                      "string(/gde2-version/minor)",
                                      "string(/gde2-version/micro)" };
            string values[3];

            if (not (document = xmlParseFile(DATADIR "/gde2-about/gde2-version.xml")))
                return;
            ...
            if (!values[0].empty() && !values[1].empty() && !values[2].empty())
                this->gde2_version = values[0] + '.' + values[1] + '.' + values[2];
        }
    };
...
procman_create_sysinfo_view(void)
{
    ...
    /* left-side logo */
    if (g_file_test (DATADIR "/pixmaps/gde2-system-monitor/distribution/side.png", G_FILE_TEST_EXISTS)) {
        logo = gtk_image_new_from_file(DATADIR "/pixmaps/gde2-system-monitor/distribution/side.png");
    }
    else {
        logo = gtk_image_new_from_file(DATADIR "/pixmaps/gde2-system-monitor/side.png");
    }
    ...
    if (data->gde2_version != "")
    {
        markup = g_strdup_printf(_("GDE2 %s"), data->gde2_version.c_str());
        header = gtk_label_new(markup);
        gtk_label_set_selectable(GTK_LABEL(header), TRUE);
        g_free(markup);
        gtk_misc_set_alignment(GTK_MISC(header), 0.0, 0.5);
