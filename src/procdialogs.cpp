...
prefs_dialog_button_pressed (GtkDialog *dialog, gint id, gpointer data)
{
    ...
    {
        GError* error = 0;
        if (!g_app_info_launch_default_for_uri("help:gde2-system-monitor/gde2-system-monitor-prefs", NULL, &error))
        {
                                       ...
                                       gint extra_value)
{
    ...
    else if (procman_has_gde2su())
        ret = procman_gde2su_create_root_password_dialog(command);
